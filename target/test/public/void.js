...
class Component {
    ...
    constructor(tag, attributes, ...content) {
        ...
        this.content = this.extractContent(content);
        this.attributes.id = this.attributes.id || randomID();
        this.container = attributes?.container || false;
    ...
    /**
     * Extract components from arrays, therefore content can be displayed for class-based-rendering as well.
     * @param {*} content component content array
    ...
    extractContent(content) {
        ...
        for (const element of content) {
            // recursively extract the components of the array if the element is an array
            if (Array.isArray(element)) {
                let extract = this.extractContent(element)
                Array.prototype.push.apply(result, extract);
            }
            // element is a normal component or value
            // make sure not to register null values
            else if (element) {
                result.push(element);
            }
        }
     ...
     */
    parse() {
        // if this component is a content holder, then return the content only
        if (this.container)
    ...
    parseAttributes() {
        ...
        // register classes from attributes
        if ('class' in this.attributes) {
            let split = this.attributes['class'].split(' ');
            Array.prototype.push.apply(classes, split);
        }
...
class Void {
    ...
    static createElement = (data, attributes, ...content) => {
        ...
        let component = new data(data.name, attributes, ...content);
